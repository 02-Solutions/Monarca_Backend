...
export class SeedService {
  ...
  constructor(
    ...
    private readonly travelAgencyRepo: Repository<TravelAgency>,
  ...
  async run() {
    const seedData: SeedData[] = [
      ...
      {
        repo: this.destinationRepo,
        file: 'destinations.json',
        entityName: 'Destination',
      },
      {
        repo: this.travelAgencyRepo,
        file: 'travel-agencies.json',
        entityName: 'TravelAgency',
      },
      ...
      { repo: this.userRepo, file: 'users.json', entityName: 'User' },
      {
        ...
        file: 'roles-permissions.json',
        entityName: 'RolePermission',
      },
      {
        repo: this.userLogsRepo,
        file: 'user-logs.json',
        entityName: 'UserLogs',
      },
      { repo: this.requestRepo, file: 'requests.json', entityName: 'Request' },
      {
        repo: this.requestsDestinationRepo,
        file: 'requests-destinations.json',
        entityName: 'RequestsDestination',
      },
      {
        repo: this.reservationRepo,
        file: 'reservations.json',
        entityName: 'Reservation',
      },
      {
        repo: this.requestLogRepo,
        file: 'request-logs.json',
        entityName: 'RequestLog',
      },
      {
        repo: this.revisionRepo,
        file: 'revisions.json',
        entityName: 'Revision',
      },
    ...
    for (const { repo, file, entityName } of seedData) {
      ...
      if (count > 0) {
        this.logger.log(
          `There are already ${entityName}s in the database. Seeding skipped.`,
        );
        continue;
      }
